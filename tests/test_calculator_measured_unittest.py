...
if 'cts_celery' in _path:
	from qed.cts_celery.cts_calcs.calculator_measured import MeasuredCalc
elif 'cts_app' in _path:
	from qed.cts_app.cts_calcs.calculator_measured import MeasuredCalc

from qed.temp_config.set_environment import DeployEnv



class TestMeasuredCalculator(unittest.TestCase):
	"""
	Unit test class for calculator_measured module, and maybe
	all the calculator_measured depending on how bloated this gets.
	"""

	print("cts calculator_measured unittests conducted at " + str(datetime.datetime.today()))
	...
	def setUp(self):
		...
		self.filename_structure = "mock_json/calculator_measured_result_{}.json"

		self.calc_obj = MeasuredCalc()
	...
	def test_makeDataRequest(self):
		...
		with patch('qed.cts_app.cts_calcs.calculator_measured.requests.post') as service_mock:
			service_mock.return_value = expected_result
			response = self.calc_obj.makeDataRequest(self.test_smiles)
	...
	@patch('qed.cts_app.cts_calcs.calculator_measured.MeasuredCalc.validate_response')
	@patch('qed.cts_app.cts_calcs.calculator_measured.requests.post')	
	...
	@patch('qed.cts_app.cts_calcs.calculator_measured.MeasuredCalc.makeDataRequest')
	@patch('qed.cts_app.cts_calcs.calculator_measured.SMILESFilter.parseSmilesByCalculator')
	def test_data_request_handler(self, smiles_filter_mock, request_mock):
		"""
		Testing measured calculator module's data_request_handler function.
		"""
