...
class Hydrolysis:
	...
	def get_qsar_for_products_epi_api(self, parent, grouped_products):
		...
		child_obj_list = self.handle_hl_response_epi_api(response_obj, parent, grouped_products)
	...
	def handle_hl_response_epi_api(self, response_obj, parent, grouped_products):
		...
		for path_key, child_obj_list in grouped_products.items():
			...
			if case == "A":
				if path == "1":
					...
					child_obj_list = self.hl_result_pattern("Ka/n", child_obj_list, response_obj)
				elif path == "2":
					...
					child_obj_list = self.handle_functional_group_case(route, parent, response_obj, child_obj_list)
				else:
					# return self.hl_result_pattern("Kb", child_obj_list, response_obj)
					child_obj_list = self.hl_result_pattern("Kb", child_obj_list, response_obj)
			elif case == "B":
				# return self.handle_op_ester_values(response_obj, child_obj_list)
				child_obj_list = self.handle_op_ester_values(response_obj, child_obj_list)
			elif case == "D":
	...
	def handle_functional_group_case(self, route, parent, response_obj, child_obj_list):
		...
		logging.info("Functional groups: {}".format(func_group))
		logging.info("Num of sites for FGs: {}".format(len(func_group)))
		logging.info("response_obj: {}".format(response_obj))
		if len(func_group) == 1:
			return self.hl_result_pattern("Kb", child_obj_list, response_obj)
		return self.assign_qualitative_values(child_obj_list)
	...
	# def make_qsar_request_epi_jar(self, request_dict):
	#     """
	#     Makes request to EPI API jar file for half-lives
	#     """
	#     parent = request_dict.get("filtered_smiles")
	#     unique_schemes_count = int(request_dict.get("uniqueSchemesCount"))  
	#     product_count = int(request_dict.get("productCount"))
